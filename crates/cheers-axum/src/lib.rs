...
//! @yah:gotcha("Warden uses this keypair to mint its OWN short-lived tokens (sub: svc:warden-<id>) signed with the Ed25519 key. Cheers verifies these on receipt at POST /ownership etc. — do not assume cheers always mints warden's tokens.")
...
//! @yah:handoff("WIRE-CONTRACT COORDINATION: this work doesn't change the W159 wire contract (claims shapes, scopes, token envelope). The JWKS shape DOES touch the constable consumer surface — F11 is where that coordination is needed, not here.")
...
//! @yah:phase(P3)
//! @yah:parent(R020)
//! @yah:next("POST /admin/camps/bootstrap { bound_to: user:<U>, desired_id, initial_grants: […] } authenticated as warden's service principal; body carries the user U's signed delegation (per yah/W122 QR-pair flow).")
//! @yah:next("Verify warden's identity + verify the user-signed delegation; allocate camp principal with bound_to: user:<U>; issue long-lived refresh credential.")
...
//! @yah:gotcha("Constable matches by kid and falls back to a one-shot rate-limited (1/sec) refresh on unknown kid. Don't change the kid format without coordinating with the constable refresh path.")
//! @arch:see(.yah/docs/working/mcp-auth-and-ownership.md)
//! @yah:depends_on(R020-F9)
...
//! @yah:handoff("CACHING: Cache-Control: public, max-age=300 + strong ETag (\"<hex(sha256(body))>\"). Conditional GET on If-None-Match: <etag> returns 304 with empty body, same Cache-Control + ETag echoed back. Mismatch falls through to 200. Cheap conditional GET works for constable's local cache invalidation.")
//! @yah:handoff("KID FORMAT: respects R020-F11's gotcha — service-principal kids are F9's mint_kid() output (128-bit base64url-no-pad opaque). Platform kids are operator-supplied; the docs call out 'use the same opaque shape'. NO structured prefixes / numeric generations — would break the constable refresh path.")
//! @yah:handoff("Jwk/JwkSet fields are String (not &'static str) so the types ROUND-TRIP through serde — a constable / product fetching JWKS can deserialize the response into the same struct cheers serializes from. Tests exercise this both ways.")
...
//! @yah:next("Propagate JWKS endpoint shape into yah-side W159 / R426 (constable verifier wiring) — endpoint is GET /.well-known/jwks.json, application/jwk-set+json, OKP+Ed25519+kid, Cache-Control: public, max-age=300, ETag-conditional. Same kid format as F9.")
...
//! @yah:handoff("WIRE-CONTRACT COORDINATION: this DOES touch consumer surface via the discovery hop yah's constable follows from /.well-known/oauth-protected-resource → cheers's issuer → this discovery doc. Endpoint URL + shape + Scope::ALL ordering need to land in yah-side W159 / R426 before the constable discovery wiring is exercised. kid format unchanged; jwks_uri points at the same /.well-known/jwks.json the F11 route serves.")
...
//! @yah:next("Propagate discovery endpoint shape into yah-side W159 / R426 (constable discovery wiring): GET /.well-known/openid-configuration, application/json, issuer/jwks_uri/token_endpoint/scopes_supported/grant_types_supported/subject_types_supported fields, scopes_supported derived from Scope::ALL ordering.")
